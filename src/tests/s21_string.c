...
/* void *s21_insert(const char *src, const char *str, s21_size_t start_index) {
    ...
    s21_size_t n_src = 0;
    int n;
    if ((src != S21_NULL) && (str != S21_NULL)) n_src = s21_strlen(src);
    void *temp;
...
/* char *s21_strstr(const char *haystack, const char *needle) {
    ...
    while ((j < n_needle) && (i < n_haystack)) {
        j = 0;
        while ((needle[j] != haystack[i]) && (i < n_haystack)) {
            i++;
        }
        int k = i;
    ...
    if ((i != 0) && (i < n_haystack)) res = (char *)haystack + i - 1;

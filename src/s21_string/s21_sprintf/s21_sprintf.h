...
void parse_field_width(const char **format, struct format_info *info,
                       va_list args);
void parse_precision(const char **format, struct format_info *info,
                     va_list args);
...
void getSEM(unsigned int *sign, unsigned int *exponent,
            unsigned long long *mantiss, double number);
